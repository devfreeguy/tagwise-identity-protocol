...
2. `pnpm --filter @tip/core build` (apps/api, @tip/moderation, and the SDK depend on it)
3. `pnpm --filter @tip/moderation build` (apps/api depends on it)
4. `pnpm --filter @tagwise/tip-sdk build` (apps/docs depends on it for typechecked code samples)
5. `pnpm --filter @tip/api build` (compiles apps/api, including the export script)
6. `pnpm --filter @tip/api export:openapi` (writes `apps/api/openapi.json`)
7. `pnpm --filter @tip/docs build` (regenerates the API Reference MDX from that file, then runs `next build`)
...
```json
{
  ...
  "buildCommand": "cd ../.. && pnpm --filter @tip/db build && pnpm --filter @tip/core build && pnpm --filter @tip/moderation build && pnpm --filter @tagwise/tip-sdk build && pnpm --filter @tip/api build && pnpm --filter @tip/api export:openapi && pnpm --filter @tip/docs build",
