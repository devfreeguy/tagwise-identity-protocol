...
export function Step1Preview({ active }: { active: boolean }) {
  return (
    <motion.div
      initial="hidden"
    ...
    >
      ...
      <motion.div
        variants={{
          ...
          visible: { opacity: 1, y: 0, transition: { delay: 1.8, duration: 0.4 } }
        }}
        className="mt-5 text-emerald-400 text-sm font-medium self-start flex items-center gap-2"
      >
        Congratulations, this tag is available!!! 🎉
      </motion.div>
    </motion.div>
  );
}
